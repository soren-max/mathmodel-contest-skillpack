...
- `data/raw/`、`data/processed/`、`src/`、`results/`、`figures/`、`notes/`、`paper/`、`materials/`、`project/`、`problem_files/` 是否齐全；原始数据保持只读。
...
结束时给出第一问需要澄清的输入/输出及 baseline 任务；如可用，建议进入 `modeling-reviewer`。此 Skill 不自动授权子代理、外部消息、提交或推送。
